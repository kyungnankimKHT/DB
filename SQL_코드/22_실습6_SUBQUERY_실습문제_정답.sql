...
FROM EMPLOYEE E
JOIN (SELECT DEPT_CODE, MANAGER_ID FROM EMPLOYEE WHERE SUBSTR(EMP_NO, 1, 2) = '77' AND SUBSTR(EMP_NO, 8, 1) IN ('2', '4', '6', '8')) T
ON E.DEPT_CODE = T.DEPT_CODE AND E.MANAGER_ID = T.MANAGER_ID;
...
JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
WHERE (E.DEPT_CODE, E.HIRE_DATE) IN (SELECT DEPT_CODE, MIN(HIRE_DATE) FROM EMPLOYEE WHERE ENT_YN = 'N' GROUP BY DEPT_CODE)
...
SELECT EMP_ID, EMP_NAME, JOB_NAME, AGE, '\' || TO_CHAR(SALARY, '999,999,999') AS 연봉포함보너스
FROM (
    SELECT E.EMP_ID, E.EMP_NAME, J.JOB_NAME, 
           EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) + 
           CASE WHEN TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) > TO_NUMBER(TO_CHAR(SYSDATE, 'YY')) THEN 1900 ELSE 2000 END AS AGE, 
           (E.SALARY * 12) AS SALARY
    FROM EMPLOYEE E
    JOIN JOB J ON E.JOB_CODE = J.JOB_CODE
    WHERE (E.JOB_CODE, EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) + 
           CASE WHEN TO_NUMBER(SUBSTR(E.EMP_NO, 1, 2)) > TO_NUMBER(TO_CHAR(SYSDATE, 'YY')) THEN 1900 ELSE 2000 END) IN 
          (SELECT JOB_CODE, MIN(EXTRACT(YEAR FROM SYSDATE) - TO_NUMBER(SUBSTR(EMP_NO, 1, 2)) + 
           CASE WHEN TO_NUMBER(SUBSTR(EMP_NO, 1, 2)) > TO_NUMBER(TO_CHAR(SYSDATE, 'YY')) THEN 1900 ELSE 2000 END)
           FROM EMPLOYEE
           GROUP BY JOB_CODE)
)
ORDER BY AGE DESC;
-- \124,800,000 대표 연봉 표기되게 해볼것 (마지막문제)